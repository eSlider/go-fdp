...
func historySampleCount(h frameHistory) int {
	n := 0
	for _, pts := range h {
		if len(pts) > n {
			n = len(pts)
...
func newRootModel(cfg appConfig, collector *polymarket.Collector) rootModel {
	...
	return rootModel{
		...
		zone:      zm,
		tab:       tabPredictions,
		preds:     newPredictionsModel(cfg, collector, zm),
...
func (m rootModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.MouseClickMsg:
		...
		} else if m.tab == tabPredictions && m.preds.HandleMouse(msg) {
			// handled
	...
	case tea.MouseMotionMsg, tea.MouseWheelMsg, tea.MouseReleaseMsg:
		if m.tab == tabPredictions && m.preds.HandleMouse(msg) {
			// handled
		} else if m.tab == tabCharts {
	...
	case tickMsg:
		if m.tab == tabPredictions {
			cmds = append(cmds, m.preds.fetchCmd(), m.preds.tickCmd())
		} else {
			cmds = append(cmds, m.charts.tickCmd())
		}
	case fetchMsg:
		if m.tab == tabPredictions {
			updated, cmd := m.preds.Update(msg)
